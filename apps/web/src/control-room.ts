...
function taskDetail(path: string, identity: ControlRoomIdentity): string {
  const id = path.split("/").slice(-1)[0] ?? "unknown";
  const canApprove = identity.permissions.includes("APPROVAL:DECIDE");
...
function operationsPage(view?: OperationsView): string {
  ...
  const alerts = view.alerts
    .map(
      (alert) =>
        `<tr><td>${status(alert.severity, alert.severity === "CRITICAL" ? "failed" : "approval")}</td><td><strong>${escapeHtml(alert.affected_system)}</strong></td><td>${escapeHtml(alert.state)}</td><td>${alert.notification ? `${status(alert.notification.state, alert.notification.state === "DELIVERED" ? "healthy" : "approval")}<br><small>${escapeHtml(alert.notification.event_kind)} · ${escapeHtml(alert.notification.last_event_at)}</small>` : "No delivery evidence"}</td><td>${escapeHtml(alert.owner_reference)}</td></tr>`,
    )
    .join("");
